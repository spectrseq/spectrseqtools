...
class DeconvolutionParameters:
    def __init__(self, parameters):
        self.averagine = parameters.get(
            "averagine", ms_ditp.Averagine(create_averageine())
        )
...
def deconvolute_scans(raw_file_read, parameters, extract_mz=True):
    ...
    df_deconvolved = df_deconvolved.sort("neutral_mass").with_columns(
        (
            1e6
    ...
    sequence_mass = (
        df_deconvolved_agg.filter(pl.col("is_precursor_deisotoped"))
